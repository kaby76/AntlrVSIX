...
namespace Trash
{
    public class MyUnbufferedTokenStream : ITokenStream
    {
        ...
        public MyUnbufferedTokenStream(ITokenSource tokenSource)
            : this(tokenSource, 256)
        {
        }

        public MyUnbufferedTokenStream(ITokenSource tokenSource, int bufferSize)
        {
...
//NO            Fill(1);
        ...
        protected internal virtual void Add(IToken t)
        {
            if (n >= tokens.Length)
            {
                tokens = MyArrays.CopyOf(tokens, tokens.Length * 2);
            }
