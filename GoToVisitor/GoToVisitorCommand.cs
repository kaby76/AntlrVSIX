...
";
            ...
            {
                var member_name = visitor ? ("Visit" + sym) : ("Enter" + sym);
                // Find point for edit.
...
public override Result {member_name}([NotNull] A3Parser.AContext context)
{{
    return VisitChildren(context);
}}
"
                    : $@"
public override void {member_name}(A3Parser.AContext context
{{
    base.EnterA(context);
}}
";
                ...
                try
                {
                    foreach (var me in found_class.Members)
                    {
                        var method_member = me as MethodDeclarationSyntax;
                        if (method_member == null) continue;
                        if (method_member.Identifier.ValueText.ToLower() == member_name.ToLower())
                        {
                            found_member = method_member;
                            throw new Exception();
                        }
