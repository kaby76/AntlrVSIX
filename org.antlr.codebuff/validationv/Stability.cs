...
//JAVA TO C# CONVERTER WARNING: Method 'throws' clauses are not available in .NET:
//ORIGINAL LINE: public static java.util.List<Nullable<float>> checkStability(org.antlr.codebuff.misc.LangDescriptor language) throws Exception
		public static IList<float?> checkStability(LangDescriptor language)
		{
			...
			for (int i = 1; i <= STAGES; i++)
			{
				...
				for (int j = 0; j < formatters.Count; j++)
				{
					...
					float editDistance = normalizedLevenshteinDistance(f.Output, f0.Output);
