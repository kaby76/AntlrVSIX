...
//ORIGINAL LINE: public static void runCaptureForOneLanguage(org.antlr.codebuff.misc.LangDescriptor language) throws Exception
	...
	//			examineCorpus(corpus);
				ListMultimap<FeatureVectorAsObject, int?> ws = getWSContextCategoryMap(fileCorpus);
				ListMultimap<FeatureVectorAsObject, int?> hpos = getHPosContextCategoryMap(fileCorpus);

				// Compare with corpus minus this file
//JAVA TO C# CONVERTER WARNING: The original Java variable was marked 'final':
//ORIGINAL LINE: final String path = new java.io.File(fileName).getCanonicalPath();
				string path = (new File(fileName)).CanonicalPath;
				IList<InputDocument> others = filter(documents, d => !d.fileName.Equals(path));
				Corpus corpus = new Corpus(others, language);
				corpus.train();
	//			examineCorpus(corpus);
				ListMultimap<FeatureVectorAsObject, int?> corpus_ws = getWSContextCategoryMap(corpus);
				ListMultimap<FeatureVectorAsObject, int?> corpus_hpos = getHPosContextCategoryMap(corpus);

				foreach (FeatureVectorAsObject x in ws.Keys)
				{
					HashBag<int?> fwsCats = getCategoriesBag(ws.get(x));
					IList<float?> fwsRatios = getCategoryRatios(fwsCats.Values);
					HashBag<int?> wsCats = getCategoriesBag(corpus_ws.get(x));
...
//ORIGINAL LINE: if (!fwsRatios.equals(wsRatios))
					if (!fwsRatios.SequenceEqual(wsRatios))
					{
						Console.WriteLine(fwsRatios + " vs " + wsRatios);
					}

					HashBag<int?> fhposCats = getCategoriesBag(hpos.get(x));
					HashBag<int?> hposCats = getCategoriesBag(corpus_hpos.get(x));
				}
		...
		public static ListMultimap<FeatureVectorAsObject, int?> getWSContextCategoryMap(Corpus corpus)
		{
			ListMultimap<FeatureVectorAsObject, int?> wsByFeatureVectorGroup = ArrayListMultimap.create();
			int numContexts = corpus.featureVectors.Count;
			for (int i = 0; i < numContexts; i++)
			{
				int[] X = corpus.featureVectors[i];
				int y = corpus.injectWhitespace[i].Value;
		...
		public static ListMultimap<FeatureVectorAsObject, int?> getHPosContextCategoryMap(Corpus corpus)
		{
			ListMultimap<FeatureVectorAsObject, int?> hposByFeatureVectorGroup = ArrayListMultimap.create();
			int numContexts = corpus.featureVectors.Count;
			for (int i = 0; i < numContexts; i++)
			{
				int[] X = corpus.featureVectors[i];
				int y = corpus.hpos[i].Value;
		...
		public static void examineCorpus(Corpus corpus)
		{
			...
			{
				int[] X = corpus.featureVectors[i];
				int y1 = corpus.injectWhitespace[i].Value;
				int y2 = corpus.hpos[i].Value;
			...
			{
				IList<int?> cats = wsByFeatureVectorGroup.get(x);
				IList<int?> cats2 = hposByFeatureVectorGroup.get(x);
			...
			Console.WriteLine("MEAN " + mean(wsEntropies));
			Console.WriteLine("MEAN " + mean(hposEntropies));
