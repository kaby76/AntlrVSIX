...
//ORIGINAL LINE: public static void main(String[] args) throws Exception
		public static void Main(string[] args)
		{
			...
			float java_guava_median;
			{
				...
				Triple<IList<Formatter>, IList<float?>, IList<float?>> sqliteResults = sqliteValidator.validateDocuments(false, null);
				Triple<IList<Formatter>, IList<float?>, IList<float?>> tsqlResults = tsqlValidator.validateDocuments(false, null);
				IList<Formatter> sqliteFormatters = sqliteResults.a;
				IList<Formatter> tsqlFormatters = tsqlResults.a;

				IList<float?> distances = new List<float?>();
				for (int i = 0; i < sqliteFormatters.Count; i++)
				{
					Formatter sqlite = sqliteFormatters[i];
					Formatter tsql = tsqlFormatters[i];
					float editDistance = normalizedLevenshteinDistance(sqlite.Output, tsql.Output);
	...
	//				System.out.println(sqlite.testDoc.fileName+" edit distance "+editDistance);
...

			{
				...
				for (int i = 0; i < javaFormatters.Count; i++)
				{
					...
					float editDistance = normalizedLevenshteinDistance(java.Output, java8.Output);
					distances.Add(editDistance);
	//				System.out.println(java.testDoc.fileName+" edit distance "+editDistance);
...

			{
				...
				for (int i = 0; i < java_guavaFormatters.Count; i++)
				{
					...
					float editDistance = normalizedLevenshteinDistance(java_guava.Output, java8_guava.Output);
					distances.Add(editDistance);
